"""Waveshare 2.13" V4 e-paper display backend.

V4 extends V3 with fast refresh support:
  - displayPartBaseImage() for full refresh
  - displayPartial() for fast updates
  - init_fast() + display_fast() for temperature-compensated fast refresh
  - Built-in temperature sensor

Resolution: 122x250, B/W, supports partial refresh + fast mode.
"""
...
class Waveshare2in13V4Display(DisplayBackend):
    """Waveshare 2.13" V4 b/w e-paper display backend."""
    ...
    def __init__(self, config: Union[DisplayConfig, Dict[str, Any]]):
        ...
        self._use_fast = _get_display_value(config, "fast_refresh", True)
        ...
        logger.info(
            f"Waveshare2in13V4 initialized: portrait {self._width}x{self._height}"
        )

    def _init_display(self) -> None:
        if epd2in13_V4 is None:
            raise RuntimeError(
                "waveshare_epd.epd2in13_V4 not available. "
        ...
        try:
            ...
            logger.info("EPD 2in13 V4 init OK")
