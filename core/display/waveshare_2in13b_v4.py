"""Waveshare 2.13" B V4 e-paper display backend (B/W/R, 122x250).
...
class Waveshare2in13BV4Display(DisplayBackend):
    """Waveshare 2.13\" B V4 (B/W/R) e-paper display backend. Full refresh only."""
    ...
    def _init_display(self) -> None:
        if epd2in13b_V4 is None:
            raise RuntimeError(
                "waveshare_epd.epd2in13b_V4 not available. "
        ...
        try:
            ...
            self._epd.init()
            self._epd.Clear()
            logger.info("EPD 2in13 B V4 init OK")
    ...
    def render_image(self, img: Image.Image) -> None:
        ...
        try:
            ...
            time.sleep(2)
            logger.debug("EPD full refresh (3-color B V4, B/W only)")
        except Exception as e:
