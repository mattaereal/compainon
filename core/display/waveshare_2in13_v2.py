"""Waveshare 2.13" V2 e-paper display backend.

V2 uses the same partial refresh API as V3:
  - displayPartBaseImage() for full refresh
  - displayPartial() for fast updates
  - init(FULL_UPDATE) / init(PART_UPDATE) for mode selection
...
class Waveshare2in13V2Display(DisplayBackend):
    """Waveshare 2.13" V2 b/w e-paper display backend."""
    ...
    def __init__(self, config: Union[DisplayConfig, Dict[str, Any]]):
        ...
        self._init_display()
        # Logical dimensions are landscape; physical panel is portrait
        ...
        logger.info(
            f"Waveshare2in13V2 initialized: logical {self._width}x{self._height} "
    ...
    def _init_display(self) -> None:
        if epd2in13_V2 is None:
            raise RuntimeError(
                "waveshare_epd.epd2in13_V2 not available. "
        ...
        try:
            ...
            logger.info("EPD 2in13 V2 init OK")
